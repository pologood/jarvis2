...
start()
{
    nohup ${JAVA} -Djava.library.path=${JARVIS_LIB_DIR}/native -Drole=${command} ${JAVA_OPTS} -cp ${JARVIS_CONF_DIR}:${JARVIS_LIB_DIR}/common/*:${JARVIS_LIB_DIR}/${command}/* ${MAIN_CLASS} > /dev/null 2>&1 &
    echo $! > ${PID}
