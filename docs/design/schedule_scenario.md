...
类型|正常逻辑|系统状态崩溃恢复重建逻辑|父任务依赖推导|重跑逻辑
--|--|--|--|
无依赖任务|crontime|检查上一次成功执行时间＋expire策略，确定还需要启动哪些|无依赖，但需要上次任务的计划调度时间|
单亲纯依赖|父任务完成就触发|检查上一次成功所依赖父任务，以及是否有新的父任务完成|需要查询上次任务的触发任务|
Offset依赖|父任务触发时检测offset范围内父任务完成列表，依赖满足后提交crontime检查时间？|检测上一次成功执行时间，确定后续还有哪些条件满足的任务应该开始执行|根据计划调度时间和offset计算|
...
### 任务调度Crontime和依赖修改，条件满足后，是否触发执行（可能重复）？

* 是否触发执行，取决于上一次完成的任务的触发条件和修改后的触发条件是否涵盖。
  * 有crontime的任务，考察crontime的间隔，如果间隔周期内已经有完成的任务，天以下间隔的，不触发，直到下一个周期，天以上间隔的，如果计划更新时间早于新的crontime时间，则要求触发，晚于修改时间的，不触发。
    * 比如天任务，当天跑过了，crontime改到更晚的时间，到点也不再触发。
    * 比如小时任务，当小时范围内跑过了，同一个小时内，不再触发。
    * 天以上周期的任务，比如一周一次的，周三跑过了，周四修改到周五触发，则本周五应该触发，周五修改到周四触发，则本周不再触发。（理论逻辑上和天以内任务如果执行同样的周期内判断策略会更简单，但是业务含义上，天以上间隔任务，这样似乎更合理一些）
    * 不考虑依赖关系变更的问题。如果只是因为依赖关系变更，新的父任务完成，依赖关系触发后，检查发现当前条件满足了，理论上不再跑，（可能需要输出log纪录，纪录这种情况）的确需要再跑，由人工触发
  * 单亲纯依赖任务，变更依赖关系，变更完如果变更后新的父任务再次满足，则触发，否则不检查新的父任务的历史情况。
    * 但是系统崩溃以后怎么办，系统状态重建时，纯依赖任务也是要检查上一次的状态的，如何判断哪些父任务是变更之前完成的，哪些是之后？这种情况，如果上一次的任务不是当前依赖任务，则应该判断上一次的计划调度时间，和当前依赖任务的已完成任务的计划调度时间。
