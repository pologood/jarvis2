...
public class JobController extends AbstractController {
    ...
    public RestResult onlineClient(@FormParam("appName") String appName, @FormParam("appKey") String appKey, @FormParam("jobName") String jobName,
            @FormParam("cronExpression") String cronExp, @FormParam("dependJobIds") String dependJobIds, @FormParam("user") String user,
            @FormParam("jobType") String jobType, @FormParam("jobCommand") String jobCommand,
            ...
            @FormParam("failedRetries") int failedRetries, @FormParam("failedInterval") int failedInterval, @FormParam("startTime") long startTime,
            @FormParam("endTime") long endTime, @FormParam("priority") int priority, @FormParam("parameters") String parameters) throws Exception {
    ...
    public RestResult edit(@FormParam("appName") String appName, @FormParam("appKey") String appKey, @FormParam("jobName") String jobName,
            ...
            @FormParam("user") String user, @FormParam("jobType") String jobType, @FormParam("jobCommand") String jobCommand,
            // @FormParam("jobContent") String jobContent,
            @FormParam("groupId") int groupId, @FormParam("rejectRetries") int rejectRetries, @FormParam("rejectInterval") int rejectInterval,
            @FormParam("failedRetries") int failedRetries, @FormParam("failedInterval") int failedInterval, @FormParam("startTime") long startTime,
            @FormParam("endTime") long endTime, @FormParam("priority") int priority, @FormParam("parameters") String parameters) throws Exception {
        ...
        if (dependJobIds != null && !dependJobIds.equals("")) {
            JSONArray dependIdsArr = new JSONArray(dependJobIds);
            for (int i = 0; i < dependIdsArr.length(); i++) {
                ModifyDependencyProtos.DependencyEntry entry = ModifyDependencyProtos.DependencyEntry.newBuilder().setJobId(dependIdsArr.getLong(i))
                        .build();
                dependEntryList.add(entry);
            }
        }
        ...
        RestServerModifyJobRequest request = RestServerModifyJobRequest.newBuilder().setAppName(appName).setJobName(jobName).setJobId(jobId)
                .setCronExpression(cronExp).setUser(user).setJobType(jobType).setContent(jobCommand).setGroupId(groupId).setPriority(priority)
                .setFailedRetries(failedRetries).setFailedInterval(failedInterval).setRejectRetries(rejectRetries).setRejectInterval(rejectInterval)
                .setStartTime(startTime).setEndTime(endTime).addAllParameters(paraList).build();
    ...
    @Path("test")
    @Produces(MediaType.APPLICATION_JSON)
    public RestResult test(@FormParam("appKey") String appKey, @FormParam("appName") String appName, @FormParam("user") String user)
            throws Exception {
