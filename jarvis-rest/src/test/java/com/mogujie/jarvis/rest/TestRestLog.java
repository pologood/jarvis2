...
public class TestRestLog {
    ...
    public void testLogRead() throws UnirestException {
        ...
        while (true) {
            HttpResponse<String> jsonResponse = Unirest.post(baseUrl + "/api/log/readResult").field("appName", "jarvis-web").field("appToken", "123")
                    .field("user", "muming").field("parameters", paramsJson).asString();

            Assert.assertEquals(jsonResponse.getStatus(), 200);
            Type type = new TypeToken<TestRestResultEntity<LogResultVo>>() {
            }.getType();
